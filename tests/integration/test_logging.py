...
Verifies that plugin actions write to the keaubnd log under the correct
program tag (kea-ubnd), and that DEBUG lines do not appear in non-verbose mode.
"""
...
def _today_log(ssh) -> str:
    return ssh(f"ls {LOG_DIR}/ | grep keaubnd | tail -1", check=False).strip()
...
def test_log_file_exists(ssh, deploy, test_log):
    ...
    assert log, f"No keaubnd log file found in {LOG_DIR}"
...
def test_start_writes_to_log(ssh, test_log):
    ...
    assert "kea-ubnd-ddns" in after or "Listening" in after, \
...
def test_log_tag_is_kea_ubnd(ssh, test_log):
    """All log lines must come from program tag 'kea-ubnd', not 'unbound*'."""
    log = _read_log(ssh, 200)
    test_log("observed", {"log_line_count": len(log.splitlines())})
    for line in log.splitlines():
        # syslog-ng format: "date host kea-ubnd[pid]: ..."
        # Reject any line whose tag contains "unbound" but not "kea-ubnd"
        if "kea-unbound" in line and "kea-ubnd" not in line:
...
def test_log_not_in_system_unbound_log(ssh, test_log):
    """kea-ubnd entries must NOT appear in the Unbound resolver log."""
    ...
    content = ssh(f"grep kea-ubnd /var/log/resolver/{unbound_log.strip()} || true",
                  check=False)
    test_log("observed", {"kea_ubnd_in_resolver_log": bool(content.strip())})
    assert not content.strip(), \
        f"kea-ubnd lines found in resolver log: {content[:200]}"
