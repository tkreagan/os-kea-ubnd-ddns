...
| V2 | `/var/run` write-churn is tolerable as a VNODE source | unbound.pid lives in busy `/var/run`; a wakeup storm burns CPU re-stat'ing every pass. | Watch `/var/run` for 60 s on an idle then busy box; count wakeups. |
| V3 | `unbound_add_host_entries()` on our `kea_sync` hook **causes a pid cycle** | The host_entries in-memory cache is only re-read on BLOCKED→NORMAL. If this write path does **not** restart Unbound/Kea, the cache goes stale while NORMAL and the override guard reads wrong data. | Trace the hook on a reservation change; confirm an Unbound (or Kea) PID actually cycles. If not → cache needs a separate invalidation (file watch or mtime check). |
| V4 | Targeted Kea lookups exist for the drain | "O(dirty) cheap" drain assumes by-name lookups. **`lease{4,6}-get-by-hostname` exists; there is NO `reservation-get-by-hostname` in Kea host_cmds** (`reservation-get` needs subnet-id + identifier/ip). | Confirm on dev box. Decide static-side fallback: drain re-resolves dynamic by-hostname (cheap) + checks a **cached** `reservation-get-all` snapshot for the static side. |
| V5 | We can observe the d2 ACK budget in tests | d2 has **no control socket** on OPNsense, so its internal failed-update counter is not queryable. | Decide now: instrument the daemon to log per-NCR receive→sendto latency; the ACK-budget test asserts on **our** latency, and cross-checks d2 logs for DROP lines. |
| V6 | The "≥1 subnet wired to us" precondition honors Kea inheritance | `ddns-send-updates` can be set **globally** and inherited; a per-subnet-only check false-refuses startup on a valid global config. | Reuse `KcaconfigController`'s existing parse; verify it resolves inheritance. |
