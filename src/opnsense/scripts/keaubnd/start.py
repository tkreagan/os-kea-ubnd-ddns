...
the appropriate daemon(8) + kea-ubnd-ddns.py command.
"""
...
def get_config():
    """Read KeaUbnd settings from config.xml. Returns dict with defaults."""
    cfg = {
        "enabled":                    "0",
        "port":                       "53535",
        "enable_tsig":                "0",
...
def main():
    ...
    if existing:
        logger.info("kea-ubnd-ddns already running (supervisor pid %s) — not starting another.",
                    existing)
        sys.exit(0)
    ...
    if _port_in_use(port):
        logger.error(
            ...
            "Run 'configctl keaubnd stop' to clear it before starting.", port)
        sys.exit(1)
